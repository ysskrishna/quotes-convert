...
```bash
git remote add upstream https://github.com/ysskrishna/quotes-convert.git
```
...
```bash
uv run pytest tests/test_converter.py::test_single_quotes_basic
```
...
- `quotes_convert-<version>-py3-none-any.whl` (wheel)
- `quotes_convert-<version>.tar.gz` (source distribution)
